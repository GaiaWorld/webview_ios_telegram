...
+ (void)sendMessage:(NSArray *)params {
    ...
    if (clazz == nil) {
        // NSLog(@"JSBundle Error = class not found");
        [WebViewJSBundle callJSError:className funcName:funcName msg:@"class not found"];
        return;
    }

    if (nativeID.intValue == 0) {
        if ([funcName isEqualToString:@"init"]) {
            [WebViewJSBundle initImpl:clazz params:params];
        } else {
            // 类方法
            ...
            if ([clazz respondsToSelector:sel]) {
                ...
                [WebViewJSBundle callJSError:NSStringFromClass(clazz) funcName:funcName msg:@"class methed funcName not found"];
            }
        }
    } else {
        // close方法
        ...
        if (object == nil) {
            ...
            [WebViewJSBundle callJSError:NSStringFromClass(clazz) funcName:funcName msg:@"object not found"];
            return;
        }
        ...
        if ([object respondsToSelector:sel]) {
            ...
            // NSLog(@"JSBundle Error = object method funcName not found");
            [WebViewJSBundle callJSError:NSStringFromClass(clazz) funcName:funcName msg:@"object method funcName not found"];
        }
    }
}
...
+ (void)initImpl: (Class)clazz params: (NSArray*)array {
    ...
    if (obj == nil) {
        [WebViewJSBundle callJSError:NSStringFromClass(clazz) funcName:@"init" msg:@"init failed"];
        return;
    }
    
    NSNumber *listenerID = [array objectAtIndex:0];
    if (listenerID == nil) {
        [WebViewJSBundle callJSError:NSStringFromClass(clazz) funcName:@"init" msg:@"listenerID not found"];
        return;
    }
    
    identity += 1;
    [dictionary setObject:obj forKey:[NSNumber numberWithInt:identity]];
    [WebViewJSBundle callJS:listenerID code:0 params:[NSArray arrayWithObjects:[NSNumber numberWithInt:identity],nil]];
}
