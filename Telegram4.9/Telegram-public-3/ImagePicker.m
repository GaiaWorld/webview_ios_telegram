...
- (void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary<NSString *,id> *)info {
    [WebViewJSBundle callJS:[NSNumber numberWithInt:callbackId] code:0 params:[NSArray arrayWithObjects: @"图片已经选择了",nil]];
}
...
- (void)imagePickerController:(TZImagePickerController *)picker didFinishPickingPhotos:(NSArray<UIImage *> *)photos sourceAssets:(NSArray *)assets isSelectOriginalPhoto:(BOOL)isSelectOriginalPhoto infos:(NSArray<NSDictionary *> *)infos {
    if (nil == photos || 0 == [photos count]) {
        [WebViewJSBundle callJS:[NSNumber numberWithInt:callbackId] code:1 params:[NSArray arrayWithObjects: @"选择图片失败",nil]];
    }else{
        if (onlyOne) {
            UIImage *image = [photos objectAtIndex:0];
            NSString *result = [ImageUtils image2base64:image];
            [WebViewJSBundle callJS:[NSNumber numberWithInt:callbackId] code:0 params:[NSArray arrayWithObjects: result,nil]];
        }
    }
}

- (void) tz_imagePickerControllerDidCancel:(TZImagePickerController *)picker{
    [WebViewJSBundle callJS:[NSNumber numberWithInt:callbackId] code:1 params:[NSArray arrayWithObjects: @"用户选择了取消!!",nil]];
}
